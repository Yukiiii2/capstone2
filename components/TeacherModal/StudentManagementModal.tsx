...
const StudentManagementModal: React.FC<StudentManagementModalProps> = ({
  ...
  const [selectedGrade, setSelectedGrade] = useState<string | null>(initialFilter.grade ?? null);
  const [selectedStrand, setSelectedStrand] = useState<string | null>(initialFilter.strand ?? null);
  ...
  /* performance modal (unchanged – only using your defaults) */
  const renderPerformanceModal = () => {
    if (!selectedStudent || !showPerformanceModal) return null;

    const data: PerformanceData = defaultPerformanceData[selectedPerformanceType];
    const anxietyColors = {
      low: { bg: "bg-white/5", text: "text-green-400", border: "border-white/20", dot: "bg-green-400", progress: 30, progressColor: "#10b981" },
      medium: { bg: "bg-white/5", text: "text-yellow-400", border: "border-white/20", dot: "bg-yellow-400", progress: 60, progressColor: "#f59e0b" },
      high: { bg: "bg-white/5", text: "text-red-400", border: "border-white/20", dot: "bg-red-400", progress: 90, progressColor: "#ef4444" },
    };
    ...
    const renderInsightSection = (title: string, items: string[], color: string) => (
      <View className={`bg-${color}-900/20 border-l-4 border-${color}-400 p-3 rounded-r-lg`}>
        <Text className={`text-sm font-medium text-${color}-400 mb-2`}>{title}</Text>
    ...
    return (
      <Modal transparent visible={showPerformanceModal} animationType="fade" onRequestClose={() => setShowPerformanceModal(false)}>
                ...
                >
                  ...
                  <View className="flex-row justify-between mb-6 space-x-4">
                    <View className="bg-white/10 border border-white/30 rounded-2xl p-5 shadow-lg flex-1 backdrop-blur-md">
                      ...
                      <View className="mb-3">
                        <Text className={`text-3xl font-bold ${data.confidenceLevel >= 80 ? "text-green-400" : "text-red-400"}`}>
                          {data.confidenceLevel}%
                        </Text>
                      </View>
                      <View className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                        <View className="h-full rounded-full bg-gradient-to-r from-green-400 to-emerald-500" style={{ width: `${data.confidenceLevel}%` }} />
                      </View>
                    </View>
                    ...
                    <View className="bg-white/10 border border-white/30 rounded-2xl p-5 shadow-lg flex-1 backdrop-blur-md">
                      ...
                      <View className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                        <View className="h-full rounded-full" style={{ width: `${currentAnxiety.progress}%`, backgroundColor: currentAnxiety.progressColor }} />
                      </View>
                    </View>
                  </View>
                  ...
                  <View className="bg-white/10 border border-white/30 rounded-2xl p-5 shadow-lg mb-6 backdrop-blur-md">
                    ...
                    <View className="space-y-5">
                      {Object.entries(data.skillMastery).map(([skill, value]) => {
                        const color = (value as number) >= 80
                        ...
                        return (
                          <View key={skill} className="space-y-2">
                            ...
                            <View className="h-2 bg-white/10 rounded-full overflow-hidden">
                              <View className={`h-full rounded-full bg-gradient-to-r ${color}`} style={{ width: `${value}%` }} />
                            </View>
                          </View>
                        );
                      })}
                  ...
                  <View className="bg-white/10 border border-white/30 rounded-2xl p-5 shadow-lg mb-6 overflow-hidden backdrop-blur-md">
                    <View className="flex-row justify-between items-center mb-6">
                      <View>
                        <Text className="text-lg font-bold text-white">Recent Tasks</Text>
                        <Text className="text-sm text-white/60 mt-0.5">Student's latest activities</Text>
                      </View>
                      <View className="px-4 bottom-2 py-1.5">
                        <Text className="text-white text-sm font-medium">{data.recentTasks.length} completed</Text>
                      </View>
                    </View>
                    <View className="space-y-3">
                      {data.recentTasks.map((task) => (
                        <View key={task.id} className="bg-white/5 border border-white/30 rounded-xl p-4 mb-3 transition-all duration-200 active:scale-[0.98] backdrop-blur-sm">
                          <View className="flex-row justify-between items-start">
                            <View className="flex-1 pr-3">
                              <Text className="font-semibold text-white mb-2">{task.title}</Text>
                              <View className="flex-row items-center">
                            ...
                            <View className="bg-white/5 border border-white/10 px-3 py-1.5 rounded-full min-w-[70px] items-center">
                              <Text className={`text-sm font-bold ${task.score >= 80 ? "text-green-400" : task.score >= 70 ? "text-yellow-400" : "text-red-400"}`}>
                                {task.score}%
                              </Text>
                            </View>
                          </View>
                          <View className="mt-3 h-1.5 bg-white/20 rounded-full overflow-hidden">
                            <View className="h-full rounded-full" style={{ width: `${task.score}%`, backgroundColor: "#a78bfa" }} />
                          </View>
                        </View>
                      ))}
                    </View>
  ...
  const renderDropdownItem = (
    ...
    >
      <Text className={`text-white ${currentValue === value ? "font-medium" : ""}`}>
        {value}
      </Text>
    </TouchableOpacity>
  );
  ...
  return (
    <Modal transparent visible={visible} animationType="fade" onRequestClose={onClose}>
      <View className="flex-1 bg-black/50">
        <View className="flex-1 mt-20 bg-[#1A1F2E]/95 backdrop-blur-3xl rounded-t-3xl p-4">
          ...
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-2xl font-bold text-white">Student Management</Text>
          ...
          <View className="flex-row gap-3 mb-4">
            ...
            <View className="flex-1 relative">
              ...
              >
                ...
                <Text className="text-white/50 text-lg">{showGradeDropdown ? "▲" : "▼"}</Text>
              </TouchableOpacity>
              {showGradeDropdown && (
                <View className="absolute z-10 w-full mt-1 bg-[#2A3142] border border-white/10 rounded-lg shadow-lg">
                  {["11", "12"].map((grade) => (
                    <View key={`grade-${grade}`}>
                      {renderDropdownItem(grade, selectedGrade, setSelectedGrade, () => setShowGradeDropdown(false))}
                    </View>
                  ))}
                </View>
              )}
            ...
            <View className="flex-1 relative">
              ...
              >
                <Text className="text-white text-sm">{selectedStrand || "Select Strand"}</Text>
                <Text className="text-white/50 text-lg">{showStrandDropdown ? "▲" : "▼"}</Text>
              </TouchableOpacity>
              {showStrandDropdown && (
                <View className="absolute z-10 w-full mt-1 bg-[#2A3142] border border-white/10 rounded-lg shadow-lg">
                  {["STEM", "HUMSS", "ABM", "GAS", "TVL"].map((strand) => (
                    <View key={`strand-${strand}`}>
                      {renderDropdownItem(strand, selectedStrand, setSelectedStrand, () => setShowStrandDropdown(false))}
                    </View>
                  ))}
                </View>
              )}
          ...
          {/* Clear Filters */}
          <TouchableOpacity className="w-full bg-violet-500 rounded-lg py-2.5 px-4 items-center mb-4" onPress={clearFilters}>
            <Text className="text-sm font-medium text-white">Clear All Filters</Text>
          </TouchableOpacity>
          ...
          <ScrollView className="flex-1">
            {filteredStudents.length > 0 ? (
                ...
                >
                  ...
                  <View className="flex-1">
                    <View className="flex-row items-baseline">
                      <Text className="font-semibold text-white text-base flex-shrink pr-2" numberOfLines={1}>
                        {student.name}
                      </Text>
                      <View className="flex-row items-center">
                        <View
                          ...
                          style={{
                            shadowColor: student.status === "active" ? "#10B981" : "#9CA3AF",
                        ...
                        />
                        <Text className={`text-xs font-medium ${student.status === "active" ? "text-green-300" : "text-gray-400"}`}>
                          {student.status.charAt(0).toUpperCase() + student.status.slice(1)}
                        </Text>
                      </View>
                    </View>
                    <Text className="text-xs text-white/80 mt-1">Grade {student.grade} • {student.strand}</Text>
                  </View>
              ...
              <View className="py-8 items-center justify-center">
                <Text className="text-gray-500 text-center mb-3">No students found matching your criteria</Text>
                <TouchableOpacity onPress={clearFilters} className="mt-2 px-4 py-2 bg-purple-100 rounded-lg">
                  <Text className="text-purple-700 font-medium">Clear filters</Text>
                </TouchableOpacity>
              </View>
            )}
          </ScrollView>
      ...
      <Modal transparent visible={showPerformanceTypeModal} animationType="fade" onRequestClose={() => setShowPerformanceTypeModal(false)}>
        <View className="flex-1 bg-black/60 justify-center items-center p-4">
          <BlurView intensity={30} tint="dark" className="w-full max-w-md rounded-2xl overflow-hidden">
            <View className="p-6 bg-[#2A3142]/90 backdrop-blur-3xl border border-white/10 rounded-2xl">
              <Text className="text-2xl font-bold text-white mb-6 text-center">Select Performance Type</Text>
              <View className="space-y-4 mb-6">
                {(["speaking", "reading"] as PerformanceType[]).map((type) => (
                  <TouchableOpacity key={type} className="p-2 bg-white/10 border border-white/30 rounded-xl active:bg-white/10 transition-colors" onPress={() => handlePerformanceTypeSelect(type)} activeOpacity={0.8}>
                    <View className="flex-row items-center">
                      <View className="w-10 h-10 rounded-full bg-white/10 items-center justify-center mr-4">
                        <Ionicons name={type === "speaking" ? "mic-outline" : "book-outline"} size={20} color="#ffffff" />
                      </View>
                      <Text className="text-white text-lg font-medium capitalize">{type}</Text>
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
              <TouchableOpacity className="mt-2 p-4 bg-white/5 border border-white/10 rounded-xl items-center active:bg-white/10 transition-colors" onPress={() => setShowPerformanceTypeModal(false)} activeOpacity={0.8}>

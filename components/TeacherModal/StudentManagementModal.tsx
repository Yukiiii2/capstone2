...
async function buildRecentTasksFromStudentProgressMinimal(
  studentId: string,
  ...
  const { data: rows, error } = await supabase
    ...
    .order("updated_at", { ascending: false, nullsFirst: false }) // ← fixed
    .limit(50);

  if (error || !rows?.length) return [];

  const seen = new Set<string>();
  const result: Array<{ id: string; title: string; date: string; score: number }> = [];

  for (const r of rows as Array<{ id: string; progress: number | null; updated_at: string }>) {
    const d = new Date(r.updated_at);
    const dayKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(
      d.getDate()
    ).padStart(2, "0")}`;
    const score = Math.max(0, Math.min(100, Math.round(r.progress ?? 0)));
    ...
    result.push({
      id: r.id,
      title: category === "speaking" ? "Speaking Task" : "Reading Task",
      date: r.updated_at,
      score,
    });
  ...
  const loadPerformance = useCallback(
    async (studentId: string, type: PerformanceType) => {
      ...
      try {
       ...
       const recentTasks = await buildRecentTasksFromStudentProgressMinimal(studentId, type, 5);

const derived: DerivedPerf = {
  ...(cached ?? getFallbackPerf(type)),
  confidenceLevel,
  anxietyLevel,
  moduleProgress,
  skillMastery,
  // ← IMPORTANT: always use DB results so you actually see the change
  recentTasks,
};
...
setPerfCache((prev) => ({
  ...
  if (selectedStudent?.id) {
    // Bust cache so we don't show the fallback list
    const key = `${selectedStudent.id}::${type}`;
    setPerfCache((prev) => {
      const next = { ...prev };
      delete next[key];
      return next;

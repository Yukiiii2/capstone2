...
const WordList = () => (
  <View className="mb-6">
    <Text className="text-white text-lg font-bold mb-2">Word List:</Text>
    <View className="flex-row flex-wrap">
      {["Cat", "Book", "School", "Morning", "Happy", "Teacher", "Friend", "Computer", "Speak", "Learn"].map((word, index) => (
        <View key={index} className="bg-white/10 rounded-lg p-3 m-1">
...
const Instructions = () => (
  <View className="mb-6">
    ...
    <View className="bg-white/5 p-4 rounded-lg">
      <Text className="text-white mb-2">1. Read each word aloud with a clear voice.</Text>
      <Text className="text-white mb-2">2. The AI will measure pronunciation and clarity.</Text>
      <Text className="text-white">3. Repeat if the feedback score is low.</Text>
    </View>
  </View>
);
...
const FocusArea = () => (
  <View className="mb-6">
    ...
    <View className="bg-purple-500/20 p-4 rounded-lg border-l-4 border-purple-500">
      <Text className="text-white">Pronunciation accuracy and clarity.</Text>
    </View>
  </View>
);
...
export default function ReadingExercise() {
  ...
  const handleStartReading = () => {
    const content = "Cat – Book – School – Morning – Happy – Teacher – Friend – Computer – Speak – Learn";
    router.push({
      pathname: "/StudentScreen/ReadingExercise/student-voice-reading-recording",
      params: { content, title: "Simple Words (Beginner)" }
    });
  };
  ...
  return (
    <View className="flex-1 bg-gray-900">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 20, paddingTop: 20 }}>
        {currentPage === 0 ? (
          <View>
            <Text className="text-2xl font-bold text-white mb-2">Simple Words (Beginner)</Text>
            <Text className="text-purple-300 mb-6">Reading Basic Level 1</Text>
            
            <View className="bg-white/5 p-6 rounded-2xl border border-white/10 mb-6">
              <Text className="text-white text-lg mb-4">
                Task: Read each word clearly and slowly.
              </Text>

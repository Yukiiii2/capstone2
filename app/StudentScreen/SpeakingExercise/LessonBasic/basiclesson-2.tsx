...
const lessonPrompt = "List an instruction sentence so that the student states their name, where they're from, and one interesting fact about them.";
const topic = "Vocal Projection & Clarity";
...
const LESSONS: LessonDetail[] = [
  {
    ...
    subtitle: "Basic • Lesson 2",
    intro:
      "Vocal projection is speaking loudly and clearly enough to be heard without shouting. Clarity ensures your words are easy to understand.",
    importance: [
      "Ensures your message reaches the audience",
      "Builds authority and confidence",
      "Avoids miscommunication",
    ],
    tips: [
      "Practice deep breathing for stronger voice",
    ...
    quiz: [
      {
        id: 1,
        question: "What is vocal projection?",
        options: ["Speaking softly", "Speaking loudly but clearly", "Whispering", "Reading fast"],
        correct: 1,
      },
      {
        id: 2,
        question: "Why is clarity important?",
        options: ["It makes speeches funny", "It makes words easy to understand", "It confuses listeners", "It makes the speech longer"],
        correct: 1,
      },
    ],
    taskBody:
      "Task: Introduce yourself (name, where you’re from, and one interesting fact about you) in 1–2 minutes.",
    rubric: [
      { 
        label: "Loudness", 
        descriptions: {
          high: "Voice is always loud and clear",
          medium: "Sometimes soft or uneven",
          low: "Often too soft or hard to hear"
        }
      },
      { 
        label: "Voice Variety", 
        descriptions: {
          high: "Voice goes up and down naturally (not flat)",
          medium: "Some change, but still a bit flat",
          low: "Always the same, sounds robotic"
        }
      },
      { 
        label: "Word Clarity", 
        descriptions: {
          high: "Words are easy to understand",
          medium: "A few unclear words",
          low: "Many words are hard to understand"
        }
      },
      { 
        label: "Confidence", 
        descriptions: {
          high: "Voice sounds strong and sure",
          medium: "Sometimes unsure or weak",
          low: "Shaky or low energy"
        }
      }
    ],
    references: [
      { url: "https://humanities.exeter.ac.uk/media/universityofexeter/collegeofhumanities/documents/Vocal_Projection_Exercises.pdf"},
      { url: "https://www.bbc.co.uk/academy/en/articles/art20130702112133652"},
      { url: "https://www.toastmasters.org/magazine/magazine-issues/2019/july/the-power-of-the-pause"}
    ]
  }
];
...
const BASIC_ORDER_INDEX_FOR_THIS = 2;        // this is Basic Lesson #2
...
const saveQuizProgress50 = async (): Promise<void> => {
  try {
    ...
    // 2) Find the module_id for Basic, order_index = 2
    ...
    // swallow errors silently for UX
  }
};
...
// Quiz Section Component (with shuffled choices + 50% progress write on Done)
const QuizSection = ({ data, onBack, onNext }: { 
  ...
  const handleDone = async () => {
    // Save 50% progress for this module, then proceed to recording
    try {
      setSaving(true);
...
export default function LessonScreen() {
  ...
  const lessonId = parseInt(params.id as string) || 2;

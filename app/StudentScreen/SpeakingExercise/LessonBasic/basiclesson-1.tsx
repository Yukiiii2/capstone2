...
const LESSONS: LessonDetail[] = [
  {
    ...
    intro:
      "Non-verbal communication includes body language, gestures, eye contact, and facial expressions. It strengthens verbal communication and helps the speaker connect with the audience.",
    importance: [
    ...
    quiz: [
      {
        ...
        options: ["Grammar rules", "Gestures and body language", "Vocabulary words", "Speechwriting"],
        correct: 1,
      },
      {
        id: 2,
        question: "Why is non-verbal communication important?",
        options: ["It distracts the audience", "It makes the speech longer", "It builds credibility", "It avoids eye contact"],
        correct: 2,
      },
    ],
    taskBody:
      "Task: Record a short 30–60 second introduction speech (Name, Age, Grade level, Hobbies).",
    rubric: [
      { 
        label: "Eye Contact", 
        descriptions: {
          high: "Looks at the camera most of the time",
          medium: "Sometimes looks away",
          low: "Rarely looks at the camera"
        }
      },
      { 
        label: "Gestures", 
        descriptions: {
          high: "Uses natural hand/body movements",
          medium: "A few stiff or awkward gestures",
          low: "No gestures or distracting movements"
        }
      },
      { 
        label: "Posture", 
        descriptions: {
          high: "Stands/sits tall and open",
          medium: "Sometimes slouches",
          low: "Often slouches, closed or tense"
        }
      },
      { 
        label: "Facial Expressions", 
        descriptions: {
          high: "Shows emotions clearly (smile, serious when needed)",
          medium: "Limited expressions",
          low: "Blank face, little emotion"
        }
      }
    ],
    references: [
      { url: "https://doi.org/10.4324/9781003054046" },
      { url: "https://www.routledge.com/Nonverbal-Communication/Mehrabian/p/book/9781138852060"},
      { url: "https://www.toastmasters.org/magazine/magazine-issues/2020/june/body-language"}
    ]
  }
];
          ...
          {!submitted ? (
            ...
            <View className="mt mb-2">
              <Text className="text-white/80 text-center mb-4">
                Your score: {Math.round((data.quiz.filter((q, i) => (shuffled[q.id] ?? [])[answers[q.id] ?? -1]?.isCorrect).length / data.quiz.length) * 100)}%
                {'\n'}
                {data.quiz.filter((q, i) => (shuffled[q.id] ?? [])[answers[q.id] ?? -1]?.isCorrect).length / data.quiz.length >= 0.7 

...
export default function CreateAccountStudent() {
  ...
  const validateStep = (step: number) => {
    if (step === 0) {
      const requiredFields: (keyof FormData)[] = ['firstName', 'lastName', 'mobileNumber', 'email', 'password', 'confirmPassword'];
      const emptyFields = requiredFields.filter((f) => !formData[f]?.trim());
      if (emptyFields.length > 0) {
        showCustomAlert('Missing Information', 'Please fill out all required fields before continuing.');
        return false;
      }
      if (formData.password !== formData.confirmPassword) {
        showCustomAlert('Validation Error', "Passwords do not match. Please make sure both passwords are the same.");
        return false;
      }
      if (formData.password.length < 8) {
        showCustomAlert('Weak Password', 'Password must be at least 8 characters long.');
        return false;
      }
      const cleaned = formData.mobileNumber.replace(/\D/g, '');
      if (!/^9\d{9}$/.test(cleaned) && !/^09\d{9}$/.test(cleaned)) {
        showCustomAlert('Invalid Mobile Number', 'Please enter a valid PH mobile (e.g., 9xxxxxxxxx or 09xxxxxxxxx).');
        return false;
      }
  ...
  const isFormComplete = () => {
    ...
    const isPasswordValid = formData.password === formData.confirmPassword && formData.password.length >= 8;
    const isVerificationValid = activeStep !== 1 || (!!selectedVerificationType && !!verificationFile);
  ...
  const renderProgressBar = () => (
    <View className="flex-row justify-center items-center mb-8">
      ...
      <View className="absolute flex-row justify-between w-full px-2">
        ...
        <View className="items-center w-24">
          <Text className={`text-xs top-3 mt-2 ${activeStep >= 1 ? 'text-violet-400 font-medium' : 'text-gray-400'}`}>VERIFY STUDENT</Text>
        </View>
        <View className="items-center w-24">
        ...
        return (
          ...
          >
            <View className="items-center mb-2">
              <Text className="text-white text-2xl font-bold mb-1">Student Registration</Text>
              <Text className="text-gray-400 text-center text-sm mb-4">Step 1 of 2: Enter your account details</Text>
              {renderProgressBar()}
            </View>
                ...
                <View className="flex-row items-center bg-white/10 border border-white/10 rounded-lg px-3 py-0.1">
                  ...
                  {field.key === 'mobileNumber' ? (
                    ...
                    <TextInput
                      ...
                      placeholderTextColor="#9CA3AF"
                      value={field.value}
                      onChangeText={(text) => setFormData({ ...formData, [field.key]: text } as any)}
                      secureTextEntry={
                        field.key === 'password'
                          ? !passwordVisible
        ...
        return (
          ...
          >
            <View className="items-center">
              <Text className="text-white text-2xl font-bold mb-1">Student Verification</Text>
              <Text className="text-gray-400 text-center text-sm mb-4">Step 2 of 2: Verify your student status</Text>
              {renderProgressBar()}
            </View>
                  ...
                  <TouchableWithoutFeedback onPress={() => setShowVerificationDropdown(false)}>
                    <View className="flex-1 bg-black/50">
                      <View
                        ...
                        style={{ transform: [{ translateY: -150 }], maxHeight: 300 }}
                      >
                        <ScrollView>
                          {VERIFICATION_OPTIONS.map((option) => (
                            ...
                            >
                              <MaterialCommunityIcons name={option.icon as any} size={20} color="#A78BFA" />
                      ...
                      <View className="bg-violet-500/20 border border-violet-400/30 rounded-lg p-2">
                        <View className="flex-row items-center justify-between">
                          ...
                          <View className="flex-row space-x-1">
                            <TouchableOpacity className="bg-white/20 p-1 rounded" onPress={() => setVerificationFile(null)}>
                              <Ionicons name="trash-outline" size={14} color="white" />
                            </TouchableOpacity>
                            <TouchableOpacity className="bg-white/20 p-1 rounded" onPress={pickVerificationDocument}>
                ...
                <View className="flex-row items-start">
                  ...
                  <View className="flex-1">
                    ...
                    <Text className="text-gray-400 text-xs">
                      We need to verify your student status to confirm you are a student.
              ...
              <View className="flex-row items-start">
                ...
                <View className="flex-1">
                  ...
                  {[
                    ...
                    { icon: 'mail', text: `Verification email sent to: ${formData.email}` },
                    {
                      content: (
                        <View>
                          <View className="flex-row items-center">
                            <Ionicons name="phone-portrait" size={14} color="#FFFFFF" style={{ marginRight: 8, marginTop: 2 }} />
                            <Text className="text-white text-medium">SMS verification sent to:</Text>
                          </View>
                          <Text className="text-white/80 text-sm ml-6">+63{formData.mobileNumber || 'your number'}</Text>
                        </View>
                      ),
                    },
                    { icon: 'document-text', text: 'Check email and SMS for updates' },
                  ].map((item, index) => (
                    <View key={index} className="flex-row items-center mb-2 last:mb-0">
                      {!item.content && (
                        <Ionicons name={item.icon as any} size={14} color="#FFFFFF" style={{ marginRight: 8, marginTop: 2, alignSelf: 'flex-start' }} />
                      )}
                      {item.content || <Text className="text-white text-medium">{item.text}</Text>}
                    </View>
                  ))}
                </View>
              </View>
  ...
  return (
    <View className="flex-1 bg-gray-900" style={{ zIndex: 1 }}>
      ...
      <BackgroundDecor />
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1" style={{ zIndex: 1 }}>
        <ScrollView
          ref={scrollViewRef}
        ...
        >
          ...
          {activeStep === 0 && (
            <View className="mt-6">
              <TouchableOpacity
                ...
                disabled={loading}
              >
                {loading ? <ActivityIndicator color="#FFFFFF" /> : <Text className="text-white font-semibold text-base">Continue</Text>}
              </TouchableOpacity>
              ...
              <View className="mt-6">
                ...
                <Text className="text-gray-400 text-xs text-center mt-2">
                  ...
                  <Text className="text-violet-300 font-medium" onPress={() => router.push('/login')}>
                    Sign in
                  </Text>
                </Text>
              </View>
          ...
          {activeStep === 1 && (
            <View className="mt-6 top-4 space-y-3" style={{ position: 'relative', zIndex: 1 }}>
              <TouchableOpacity
                className={`py-3 rounded-lg items-center justify-center -top-10 w-full max-w-[320px] mx-auto ${
                  isFormComplete() ? 'bg-violet-600/80 active:bg-violet-700/80' : 'bg-gray-600/50'
                }`}
                style={{ zIndex: 1 }}
                onPress={handleSignUp}
                disabled={!isFormComplete() || loading}
              >
                {loading ? <ActivityIndicator color="#FFFFFF" /> : <Text className="text-white font-semibold text-base">Submit Application</Text>}
              </TouchableOpacity>
              <TouchableOpacity

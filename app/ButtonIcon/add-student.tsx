...
// color chip for strand (kept from new UI)
...
const safeStrand = (s: string | null | undefined) =>
  s === "HUMMS" ? "HUMSS" : s || "";

/* ──────────────────────────────────────────────────────────────────────────
   Student Card (new UI visual, wired to logic)
   ────────────────────────────────────────────────────────────────────────── */
const StudentCard = ({
  student,
  onSelect,
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.8}
      className={`bg-white/5 backdrop-blur-xl rounded-xl p-4 mb-3 border ${
        isSelected
          ? "border-purple-400"
          : "border-white/10"
      } shadow-sm`}
...
/* ──────────────────────────────────────────────────────────────────────────
   Main Screen (merged logic + new visuals)
   ────────────────────────────────────────────────────────────────────────── */
export default function StudentApprovalScreen() {
  ...
  const requestByIdRef = useRef<
    Record<string, JoinRequestRow>
  >({});

  // DB table name for join requests
  ...
  /* ─────────────────────────────
     2. map DB rows → Student UI
     ───────────────────────────── */
  const mapToStudentUI = useCallback(
    (
      rows: JoinRequestRow[],
      profiles: ProfileRow[]
    ): Student[] => {
      ...
      return rows
        ...
        .map((r) => {
          ...
          const displayName = (
            prof?.name || "Unknown Student"
          ).trim();
          return {
            id: r.id, // join request id
            name: displayName,
            grade: r.grade_level || "",
  ...
  const fetchPending = useCallback(
    async () => {
      ...
      setLoading(true);
      try {
        // get join requests for this teacher
        const {
          data: reqs,
          error: reqErr,
        } = await supabase
        ...
        console.warn(
          "[add-student] fetchPending error:",
          e
        );
        setStudents([]);
        setSelectedStudents(
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(
        `class_join_requests:teacher:${teacherId}`
      )
      .on(
        "postgres_changes",
        {
  ...
  /* ─────────────────────────────
     6. approve / decline logic
     ───────────────────────────── */
  const approveBatch = useCallback(
    async (ids: string[]) => {
      if (!teacherId || ids.length === 0)
        return;
  ...
  const declineBatch = useCallback(
    async (ids: string[]) => {
      if (!teacherId || ids.length === 0)
        return;
  ...
  // these are kept if you want per-row action buttons later
    ...
    <View className="flex-1 bg-[#0F172A]">
      ...
      <View className="pt-10 pb-4 px-6">
        ...
        <Text className="text-white/60 text-sm mb-3 text-center">
          {loading
            ? "Loading…"
            : `${pendingStudents.length} pending student${
